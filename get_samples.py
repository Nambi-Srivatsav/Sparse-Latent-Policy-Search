...
def get_samples(W=None,
                M=None,
                tau=None,
                Latent=None,
                dimensions_per_group=None,
                Time=None,
                rendering=0,
                *args,
                **kwargs):
    ...
    for t in arange(0, Time).reshape(-1):
        for m in arange(0, number_of_groups).reshape(-1):
            ...
            xx = scipy.stats.norm.rvs(0,
                                      inv([[tau[m][0]]])[0][0] + 2,
                                      (dimensions_per_group[m], BasisDim))
            xx = xx / 2
            time.sleep(0.05)
            Actions[startDim:(startDim + dimensions_per_group[m]), t] = dot(
                dot(W[m][0], Z), Basisfunctions[:, t]) + dot(
                    M[m][0], Basisfunctions[:, t]) + dot(
                        xx, Basisfunctions[:, t])
        ...
        try:
            ...
            writing_main_data = [[
                "-110.08", "-128.668", "51.0821", "-3.07672", "56.8406",
                "-159.638", "50.8102"
            ]]
            original_angles = [
                "-111.254", "-134.357", "-0.0150254", "1.35811", "110.899",
                "-164.631", "47.7262"
            ]
